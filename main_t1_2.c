...
void forwardTurn(int dir, float target_angle, float target_rpm, int delay){
	...
	if (dir == LEFT){
		htim12.Instance->CCR1 = SERVO_LEFT;
		servo_prev = SERVO_LEFT;
	}
	else{
		htim12.Instance->CCR1 = SERVO_RIGHT;
	...
	for (;;){
		...
		//--------------- PID Controller -------------------------
		  if (dir == RIGHT){
			  ...
			  error_left_rpm = (target_rpm-25.5f) - motor_left_rpm;
			  error_right_rpm = (target_rpm) - motor_right_rpm;
		  }
...
void motors(void *argument)
{
  /* USER CODE BEGIN motors */
	...
	osDelay(1000);//delay for ultrasonic init
//	for (;;){
//		osDelay(10);
...
//		}
//	}
//	forward_until(100.0f, 15.0f, 40, 0);
	//-------------------------------


	//---------- Testing --------------
	//currently, diameter = 63~65
//	forwardTurn(LEFT, 180.0f, 60.0f, 40);

//	reverse(100.0f, 100.0f, 40, 0);
	//---------------------------------
...
//	  }
//
//	  osDelay(1000); //Time delay to recognise image
//
//	  reverse(60.0f, 20.0f, 40, 0);
//	  osDelay(300);
//
...
//	  osDelay(300);
//
//	  forward(100.0f, 36.0f, 40, 0);
//	  osDelay(500);
//	  reverseTurn(LEFT, 88.0f, 60.0f, 40);
//	  osDelay(300);
//
//	  forward_until(60.0f, 15.0f, 40, 0);
//	  osDelay(1000);
//
//	  count++;
//	  if (count == 3){
...
//	  }
	  ...
	  if (!have_instruction){
		  ...
		  if (i_angle == 0){  //forward or backward
			  ...
			  if (i_flag == 'T') forwardTurn(SERVO_RIGHT, i_angle, 60.0f, 40);
			  else				 reverseTurn(SERVO_RIGHT, i_angle, 60.0f, 40);
		  }

		  else{ //turn left
			  if (i_flag == 'T') forwardTurn(SERVO_LEFT, i_angle, 60.0f, 40);
			  else				 reverseTurn(SERVO_LEFT, i_angle, 60.0f, 40);
		  }
